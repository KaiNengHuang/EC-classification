...
def init():
    ...
    importantLightgbmFeat = range(6670)
...
def drow_roc():
    ...
    for train_index, test_index in kfold.split(data, label):
        ...
        gbm = joblib.load(str(r"../visualizationAnalysis/FC/lightgbm" + str(cnt) + ".pkl"))
        pred_y = gbm.predict_proba(T_test)
        fpr, tpr, threshold = metrics.roc_curve(test_y, pred_y[:, 1])
        fprs.append(fpr)
        tprs.append(tpr)
    ...
    # 显示与保存图
    plt.tight_layout()
    plt.savefig(f'../../data/lgb_roc.tif', dpi=500, format='tif')
    plt.show()

    # 保存当前模型得到平均fpr与tpr
    dict = {"fpr": mean_fpr, "tpr": mean_tpr, "auc": mean_auc}
    df = pd.DataFrame(dict)
    df.to_csv("../../data/lgb_fpr_tpr.csv", index=False)
