...
def run():
    ...
    for train_index, test_index in kfold.split(data, label):
        ...
        XGBoost_Accuracy.append(dictScore['weighted avg']['recall'])
        XGBoost_Sensitivity.append(dictScore['1']['recall'])
        XGBoost_Specificity.append(dictScore['0']['recall'])

        joblib.dump(classifier, "../visualizationAnalysis/xgb" + str(cnt) + ".pkl")
    ...
    print("REST_meta_MDD XGBoost five-fold cross-validation average Accuracy:%.4f"%(np.mean(XGBoost_Accuracy)))
    print("REST_meta_MDD XGBoost five-fold cross-validation average Sensitivity:%.4f"%(np.mean(XGBoost_Sensitivity)))
    print("REST_meta_MDD XGBoost five-fold cross-validation average Specificity:%.4f"%(np.mean(XGBoost_Specificity)))
    ...
    # 显示与保存图
    plt.tight_layout()
    plt.savefig(f'../../data/xgb_roc.tif', dpi=500, format='tif')
    plt.show()

    # 保存当前模型得到平均fpr与tpr
    dict = {"fpr": mean_fpr, "tpr": mean_tpr, "auc": mean_auc}
    df = pd.DataFrame(dict)
    df.to_csv("../../data/xgb_fpr_tpr.csv", index=False)
